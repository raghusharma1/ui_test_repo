...
test('login_analysis_happy_path', async ({ page, context }) => {
  const { UI_SITE_USERNAME, UI_SITE_PASSWORD, LOGIN_URL, BASE_URL, LOGIN_HOST_URL, BASE_HOST_URL } = process.env;
  ...
  try {
    ...
    // Step 1: Enter a valid username into the username field.
    // Captured selectors:
    ...
    await page.getByTestId('username-input').fill(UI_SITE_USERNAME);
    ...
    await page.getByTestId('password-input').fill(UI_SITE_PASSWORD);

    // Step 3: Click the login button to submit the credentials.
    // Captured selectors:
    ...
    const expectedUrl = `${LOGIN_HOST_URL || BASE_HOST_URL}/#/home`;
